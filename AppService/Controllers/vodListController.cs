...
{
    [RoutePrefix("api/vodList")]
    public class vodListController : ApiController
    {
                ...
                if (dbconn.idbCheck(out dbres))
                {
                    ...
                    if (dbconn.checkToken(token, out userCardNo, out userAdminNo))
                    {
                        ...
                        if(dt.Rows.Count !=0)
                        {
                            ...
                            DataTable dtOrdered = dbconn.getTable(appServiceQry._getOrderedMovie(userCardNo, contentId));
                            if(dtOrdered.Rows.Count !=0)
                            {
                                isOrdered = true;
                            }
                            else
