...
MTA_TRANSACTION_ADDITIONAL bb where AA.BILLID = BB.BILL_ID ORDER BY AA.CREATEDATE DESC", cardNo, DateTime.Today.ToString("yyyy"));
        ...
        public static string _getVODComing()
        {
            ...
            string qry = string.Format(@"SELECT NAME_MON AS CONTENT_NAME, IMAGE_WEB AS CONTENT_IMG_URL, ID, CONTENT_DESCR, TRAILER   FROM TV_CONTENT WHERE ID IN (SELECT DISTINCT ID FROM TV_PROGRAM WHERE PRODUCT_ID = 65 AND IN_DATE = TO_CHAR(SYSDATE, 'YYYYMMDD') AND IS_DELETED = 'N')");
            return qry;
        }
