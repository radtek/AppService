...
namespace sshDBWork.Utils
{
    public class LogWriter
    {
        ...
        public static void _ssh(string TAG, string logtxt)
        {
            try
            {
                var logfolder = AppDomain.CurrentDomain.BaseDirectory + "\\Log\\SSH";
                System.IO.DirectoryInfo di = new System.IO.DirectoryInfo(logfolder);
                FileSystemAccessRule fsar = new FileSystemAccessRule("Users", FileSystemRights.FullControl, AccessControlType.Allow);
                DirectorySecurity ds = null;
                if (!di.Exists)
                ...
                ds = di.GetAccessControl();
                ds.AddAccessRule(fsar);
                StreamWriter sw = new StreamWriter(logfolder + "\\ssh" + DateTime.Today.ToString("yyyyMMdd") + ".txt", true);
                sw.WriteLine(string.Format(@"{0} [{1}] {2}", DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss"), TAG, logtxt));
                sw.Close();
                logtxt = string.Empty;
            }
